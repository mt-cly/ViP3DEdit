...
def sample(
    input_path: str = "/data0/liyi/code/DGE/edit_video_vis/cond_img_1.png",  # Can either be image file or folder with image files
    ...
    elevations_deg: float = 10.0,  # For SV3D
    ...
    if version == "svd":
        num_frames = default(num_frames, 8)
        num_steps = default(num_steps, 25)
        output_folder = default(output_folder, "outputs/simple_video_sample/svd/")
        model_config = "scripts/sampling/configs/svd.yaml"
    elif version == "svd_xt":
        num_frames = default(num_frames, 5)
        num_steps = default(num_steps, 25)
    ...
    for input_img_path in all_img_paths:
        ...
        with torch.no_grad():
            with torch.autocast(device):
                ...
                imageio.imwrite(
                    os.path.join(output_folder, f"{img_name}.jpg"), input_image
                )
                ...
                video_path = os.path.join(output_folder, f"{img_name}.mp4")
                imageio.mimwrite(video_path, vid)
                latent_path = os.path.join(output_folder, f"{img_name}.pth")
                torch.save(samples_z, latent_path)

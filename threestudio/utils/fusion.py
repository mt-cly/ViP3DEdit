...
def visualize_correspondences(src_img, ref_img, srcs2ref_idx_img, num_points=10):
    import numpy as np
    ...
    srcs2ref_idx_img = np.array(srcs2ref_idx_img)
    ...
    combined_img[:, :w, :] = np.repeat(src_img, 3, axis=2)  # 将src_img复制到左侧
    combined_img[:, w:, :] = np.repeat(ref_img, 3, axis=2)  # 将ref_img复制到右侧

    # 随机选择一些点
    points = random.sample(range(h * w), num_points)

    for point in points:
        y, x = divmod(point, w)  # 将一维索引转换为二维坐标
        ref_y, ref_x = srcs2ref_idx_img[y, x]  # 获取对应的ref_img坐标

        # 确保坐标在范围内
        if 0 <= ref_y < h and 0 <= ref_x < w:
            # 在src_img上绘制一个红色点
            combined_img[y, x, :] = [255, 0, 0]  # 红色点
            # 在ref_img上绘制一个蓝色点
            combined_img[ref_y, ref_x + w, :] = [0, 0, 255]  # 蓝色点

            # 连接src_img和ref_img之间的线
            plt.plot([x, ref_x + w], [y, ref_y], color='green', linewidth=1)
...
def get_reproj(ref_depth, srcs_depth, ref_cam, srcs_cam):  # n1hw, nv1hw -> n1hw
    ...
    idx_img = get_pixel_grids(h, w).unsqueeze(0).to(srcs_depth_f.device)  # 1hw31
    ...
    srcs2ref_xyd = torch.cat([srcs2ref_idx_img[..., :2, 0], srcs2ref_idx_cam[..., 2:3, 0]], dim=-1)  # N3hw
...
def vis_filter_dynamic(ref_depth, reproj_xyd, dist_base=4, rel_diff_base=1300):
    ...
    relative_dist_thred = torch.arange(2, v + 1).reshape(1, 1, -1, 1, 1).repeat(n, v, 1, 1, 1).to(
        device) / rel_diff_base

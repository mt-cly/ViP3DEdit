...
from threestudio.utils.pixel_correspondance_utils import *
...
def compute_depth_constrains(cam1, depth1, cam2, depth2, current_H=384, current_W=512):
    '''
    args:
        cam1: the reference camera
        depth1: the depth in reference camera [h, w, 1]
        cam2: the source cameras list
        depth2: the source depths [num_view, h, w, 1]
    return:
        constraint: bool matrix with shape of [num_view*current_H*current_W, current_H*current_W] False means correspondence
    '''
    h, w, _ = depth1.shape
    intrinsic = np.array([
        [fov2focal(cam1.FoVx, cam1.image_width), 0, cam1.image_width/2],
        [0, fov2focal(cam1.FoVy, cam1.image_height), cam1.image_height/2],
        [0, 0, 1]
    ])
    cam1_matrix = np.zeros(shape=(2, 4, 4), dtype=np.float32)
    extrinsic = getWorld2View(cam1.R, cam1.T)
    cam1_matrix[0, :4, :4] = extrinsic
    cam1_matrix[1, :3, :3] = intrinsic
    cam1_matrix = torch.tensor(cam1_matrix).to(depth1.device)
    cam2_matrix = []
    for sub_cam in cam2:
        sub_cam_matrix = np.zeros(shape=(2, 4, 4), dtype=np.float32)
        extrinsic = getWorld2View(sub_cam.R, sub_cam.T)
        sub_cam_matrix[0, :4, :4] = extrinsic
        sub_cam_matrix[1, :3, :3] = intrinsic
        cam2_matrix.append(sub_cam_matrix)
    cam2_matrix = torch.tensor(cam2_matrix).to(depth2.device)

    idx_on_ref = get_reproj(depth1.permute(2, 0, 1)[None], depth2.permute(0, 3, 1, 2)[None], cam1_matrix, cam2_matrix) # [n, h, w, 2]
    depth1_ds = F.interpolate(depth1.permute(2,0,1)[None], (current_H, current_W), mode='nearest')[0]
    idx_on_ref_ds = F.interpolate(idx_on_ref.permute(0,3,1,2), (current_H, current_W), mode='nearest')
    rows = (idx_on_ref_ds[:, 1] / h * current_H).int()
    cols = (idx_on_ref_ds[:, 0] / w * current_W).int()
    row_col_index = (rows * current_W + cols).flatten()
    valid = ((rows>=0) & (rows<current_H) & (cols>=0)  & (cols < current_W)).flatten()
    valid =  valid & (abs(idx_on_ref_ds[:, 2].flatten() - depth1_ds.flatten()[row_col_index.clamp(0, current_H * current_W-1)]) < 1)
    constraint = torch.ones(depth2.shape[0] * current_H * current_W, current_H * current_W)
    constraint[torch.arange(depth2.shape[0] * current_H * current_W)[valid.cpu()], row_col_index[valid]] = 0
            ...
            if not self.use_normal_attn:
                if self.pivotal_pass:
                    ...
                    if len(batch_idxs) == 2:
                        ...
                        idx1_sum = idx1_epipolar.sum(dim=-1)
                        idx1_epipolar[idx1_sum == sequence_length, :] = False
                        sim[idx1_epipolar] = 0 # make the non-similiarity
                        sim_max = sim.max(dim=-1)
                        idx1.append(sim_max[1])
